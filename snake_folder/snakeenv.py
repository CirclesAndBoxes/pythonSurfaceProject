...
class Snekenv(gym.Env):
    ...
    def step(self, action):
        ...
        self.observation = [head_x, head_y, apple_delta_x, apple_delta_y, snake_length] + self.prev_actions
        self.observation = np.array(self.observation)

        info = {}
        return self.observation, self.reward, self.done, info

    # Once the snake bumps into the end, reset.
    def reset(self):
        self.done = False

        # Can I get away with not using this line?
        self.img = np.zeros((500, 500, 3), dtype='uint8')
        ...
        # Stuff to observe and keep it a fixed size: head_x, head_y, apple_x, apple_y, snake_length, previous moves
        # Below is the observations and reward
        head_x = self.snake_head[0]
        head_y = self.snake_head[1]
